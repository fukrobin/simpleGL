...
public class ImGuiImplGlfw {
   ...
   *
   * @param windowId pointer to the window
   * @param offsetX  scroll offset by x-axis
   * @param offsetY  scroll offset by y-axis
   */
  public void scrollCallback(final long windowId, final double offsetX, final double offsetY) {
    if (prevUserCallbackScroll != null && windowId == windowPtr) {
      prevUserCallbackScroll.invoke(windowId, offsetX, offsetY);
    }
  ...
  public void monitorCallback(final long windowId, final int event) {
    if (prevUserCallbackMonitor != null) {
      prevUserCallbackMonitor.invoke(windowId, event);
    }
  ...
  /**
   * Method to do an initialization of the {@link imgui.glfw.ImGuiImplGlfw} state. It SHOULD be
   * called before calling the {@link imgui.glfw.ImGuiImplGlfw#newFrame()} method.
   * <p/>
   ...
   * imgui.glfw.ImGuiImplGlfw#init(long, boolean)}.
   */
  public void dispose() {
    shutdownPlatformInterface();

    if (callbacksInstalled) {
      Callback.free(memAddressSafe(glfwSetWindowFocusCallback(windowPtr,
                                                              prevUserCallbackWindowFocus)));
      Callback.free(memAddressSafe(glfwSetCursorEnterCallback(windowPtr,
                                                              prevUserCallbackCursorEnter)));
      Callback.free(memAddressSafe(glfwSetMouseButtonCallback(windowPtr,
                                                              prevUserCallbackMouseButton)));
      Callback.free(memAddressSafe(glfwSetScrollCallback(windowPtr, prevUserCallbackScroll)));
      Callback.free(memAddressSafe(glfwSetKeyCallback(windowPtr, prevUserCallbackKey)));
      Callback.free(memAddressSafe(glfwSetCharCallback(windowPtr, prevUserCallbackChar)));
  ...
  private void updateMousePosAndButtons() {
    ...
    final ImGuiPlatformIO platform = ImGui.getPlatformIO();

    for (int n = 0; n < platform.getViewportsSize(); n++) {
      final ImGuiViewport viewport = platform.getViewports(n);
      ...
      if (io.getWantSetMousePos() && focused) {
        glfwSetCursorPos(windowPtr, mousePosBackup.x - viewport.getPosX(),
  ...
  private void updateMouseCursor() {
    ...
    for (int n = 0; n < platform.getViewportsSize(); n++) {
      final long windowPtr = platform.getViewports(n).getPlatformHandle();
      ...
      if (imguiCursor == ImGuiMouseCursor.None || io.getMouseDrawCursor()) {
        ...
        glfwSetCursor(windowPtr, mouseCursors[imguiCursor] != 0 ? mouseCursors[imguiCursor]
  ...
  private void updateMonitors() {
    ...
    for (int n = 0; n < monitors.limit(); n++) {
      ...
      if (glfwHasMonitorWorkArea) {
        glfwGetMonitorWorkarea(monitor, monitorWorkAreaX, monitorWorkAreaY, monitorWorkAreaWidth,
      ...
      platform.pushMonitors(mainPosX, mainPosY, mainSizeX, mainSizeY, workPosX, workPosY,
  ...
  private void windowPosCallback(final long windowId, final int posX, final int posY) {
    final ImGuiViewport vp = ImGui.findViewportByPlatformHandle(windowId);
  ...
  private final class CreateWindowFunction extends ImPlatformFuncViewport {
    ...
    public void accept(final ImGuiViewport vp) {
      ...
      data.window = glfwCreateWindow((int) vp.getSizeX(), (int) vp.getSizeY(), "No Title Yet", NULL,
                                     windowPtr);
      ...
      if (data != null && data.windowOwned) {
        ...
        for (int i = 0; i < keyOwnerWindows.length; i++) {
          if (keyOwnerWindows[i] == data.window) {
            keyCallback(data.window, i, 0, GLFW_RELEASE,
  ...
  private final class SetWindowSizeFunction extends ImPlatformFuncViewportImVec2 {
    ...
    public void accept(final ImGuiViewport vp, final ImVec2 imVec2) {
      ...
      if (IS_APPLE && !glfwHasOsxWindowPosFix) {
        glfwGetWindowPos(data.window, posX, posY);
        glfwGetWindowSize(data.window, width, height);
        glfwSetWindowPos(data.window, posX[0], posY[0] - height[0] + (int) imVec2.y);
      }
  ...
  private void initPlatformInterface() {
    @SuppressWarnings("CheckStyle") final ImGuiPlatformIO platformIO = ImGui.getPlatformIO();
    ...
    platformIO.setPlatformGetWindowMinimized(
        new GetWindowMinimizedFunction());
