...
vec3 calcParallelLight(ParallelLight light, vec3 normal, vec3 viewDir) {
    ...
    // 合并结果
    vec3 diffuseSample = texture(material.diffuseMap0, TextureCoordinates).rgb;
    vec3 ambient  = light.ambient  * diffuseSample;
    vec3 diffuse  = light.diffuse  * diff * diffuseSample;
    vec3 specular = light.specular * spec * vec3(texture(material.specularMap0, TextureCoordinates));
...
vec3 calcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    vec3 diffuseSample = texture(light.diffuseMap, TextureCoordinates).rgb;
    vec3 specularSample = texture(material.specularMap0, TextureCoordinates).rgb;
